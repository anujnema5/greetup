...
export function RoomVideoStageOverlays({
  ...
  const showLocalFloatingPreview =
    !isGroupRoom &&
    !activeActivity &&
    stageRatio !== "1:1" &&
    !participantVideosInSidebar &&
    !shareStageImmersive;
  const showPeerCameraInset =
    !isGroupRoom &&
    mainStageShowsScreen &&
    Boolean(peerCameraInsetStream) &&
    !activeActivity &&
    stageRatio !== "1:1" &&
    !participantVideosInSidebar;
  ...
  return (
    <>
      ...
      {showLocalFloatingPreview ? (
        <div
          className="pointer-events-none absolute z-20 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_10px_30px_rgba(10,12,20,0.22)] bottom-22 right-2 w-60 aspect-video xl:hidden"
        >
          {localVideoLive ? (
            <VideoMirror
      ...
      {showPeerCameraInset ? (
        <div
          className={cn(
            "pointer-events-none absolute z-20 overflow-hidden rounded-xl border border-white/20 bg-black/80 shadow-lg bottom-18 right-42 w-36",
        ...
        >
          ...
          <div
            className="absolute bottom-1.5 left-1.5 rounded px-1.5 py-0.5 text-[10px] font-semibold text-white/90 bg-black/60 backdrop-blur-xs"
          >
            {peerLabel}
          </div>
        </div>
      ...
      {scoreLabel != null ? (
        <div
          className="pointer-events-none absolute bottom-16 left-4 z-10 flex items-center gap-1.5 rounded-full bg-black/40 backdrop-blur-sm px-2.5 py-1 border border-[oklch(88%_0.11_105/0.2)]"
        >
