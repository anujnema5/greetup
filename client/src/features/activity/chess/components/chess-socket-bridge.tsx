...
export function ChessSocketBridge() {
  ...
  useEffect(() => {
    ...
    const onDrawOffered = (payload: unknown) => {
    ...
    const onDrawRejected = (payload: unknown) => {
  ...
  }, [socket, activeRealtimeActivity, activeRoomId, dispatch, isVideoSessionActive, session?.user?.id]);
