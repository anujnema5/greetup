...
const VIBES = ["design", "startups", "indie music", "ai & ml", "travel", "coffee"];
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <main className="flex flex-1 flex-col overflow-y-auto pb-16 md:pb-0">
        ...
        <header className="sticky top-0 z-50 flex items-center justify-between px-4 md:px-8 py-4 border-b border-border bg-background shadow-sm">
          ...
          <button className="flex items-center gap-1.5 rounded-xl border border-border px-3 py-2 text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors duration-200 cursor-pointer">
            <Edit2 size={13} />
            Edit
          </button>
        </header>
        ...
        <div className="flex flex-col gap-5 px-4 md:px-8 py-5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><MapPin size={11} /> Mumbai, IN</span>
                  <span className="flex items-center gap-1"><LinkIcon size={11} /> circlo.app/@anuj</span>
                </div>
              </div>
              ...
              >
                ...
                <span className="text-lg font-black" style={{ color: "oklch(88% 0.11 105)" }}>87</span>
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {VIBES.map((v) => (
                <span
                  ...
                  style={{ borderColor: "oklch(88% 0.11 105 / 0.25)", color: "oklch(88% 0.11 105)", background: "oklch(88% 0.11 105 / 0.08)" }}
                >
                  {v}
                </span>
              ))}
          ...
          <div className="grid grid-cols-4 gap-3">
            {STATS.map(({ label, value }) => (
              <div key={label} className="flex flex-col items-center gap-1 rounded-2xl border border-border bg-card py-3 px-2">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {RECENT_MATCHES.map((m) => (
                ...
                >
                  <div className={cn("h-10 w-10 rounded-full bg-gradient-to-br flex items-center justify-center text-sm font-bold text-white shrink-0", m.grad)}>
                    {m.initials}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <button className="shrink-0 flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted transition-colors duration-150 cursor-pointer">
                    <Video size={11} />
                    Call
                  </button>
                </div>
          ...
          <div className="rounded-2xl border border-border bg-card px-5 py-4">
            ...
            <div className="flex flex-col gap-3 text-sm">
              {[
                { action: "Matched with Zara K.",        time: "2h ago",  icon: Users },
                { action: "Joined Tech & Startups circle", time: "5h ago", icon: Users },
                { action: "New connection: Mia C.",       time: "1d ago",  icon: Users },
