...
export const handleKickPeer = async (c: Context): Promise<Response> => {
  ...
  const targetUserId = typeof rec.targetUserId === "string" ? rec.targetUserId : "";
  if (!roomId || !targetUserId) {
    return c.json({ ok: false as const, error: "validation" }, 400);
  }

  const result = await peers.kickPeer(roomId, targetUserId);
