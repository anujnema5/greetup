 ...
 *
 * **Step 1 (this module):** Lazy auto-start — no cron. On join / RTC token, if the space is
 ...
 *
 * **Step 3:** `deleteSpaceAfterCall` — last active participant `leave-space-rtc` ends the live space
 * immediately when the flag is on; when off, last leave only records `left_at` (circle stays live
 * until host **end space for everyone** or `expires_at` / scheduled join grace). `POST /room/:id/host-end-space`
 * ends the session for everyone.
 */
export async function maybeAutoStartScheduledSpaceFromDb(roomId: string): Promise<void> {
  const room = await roomsRepository.findRoomById(roomId);
  if (!room || room.roomType !== "space") return;
  ...
  logger.info("Scheduled space auto-started (lazy)", {
    roomId: row.id,
