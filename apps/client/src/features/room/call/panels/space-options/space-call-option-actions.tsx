...
export type RoomSpaceCallOptionActionsProps = {
  roomId: string;
  showInvite: boolean;
  onInvite?: () => void;
  showChat: boolean;
...
/** Invite / copy join link / open chat — used inside `RoomSpaceCallOptionsDialog`. */
export function RoomSpaceCallOptionActions({
  roomId,
  showInvite,
  onInvite,
  showChat,
  onOpenChat,
}: RoomSpaceCallOptionActionsProps) {
  const copyJoinLink = () => {
    if (typeof window === "undefined") return;
    const url = `${window.location.origin}/space/${encodeURIComponent(roomId)}`;
  ...
  return (
    <section className="flex flex-col gap-2" aria-label="Space actions">
